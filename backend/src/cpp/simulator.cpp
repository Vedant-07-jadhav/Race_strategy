...
void RaceSimulator::add_car(const std::string &name, StrategyWithGap strategy, double initial_fuel) {
    CarInternal car;
    car.name = name;
...
void RaceSimulator::print_results() {
    ...
    for (int i = 0; i < (int)internal_cars.size(); ++i) {
        ...
        std::cout << i + 1 << ". " << c.name << " | Time: " << c.state.total_time << "s\n"
                  << "   Interactions: DRS(" << c.stats.drs_count << ") SLIP(" << c.stats.slipstream_count << ") DIRTY(" << c.stats.dirty_air_count << ")\n"
                  << "   Net Delta: -" << c.stats.total_time_gained_drs_slip << "s / +" << c.stats.total_time_lost_dirty_air << "s\n"
                  << "-------------------------------------------\n";
    }
}